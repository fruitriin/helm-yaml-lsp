 ...
 */
export function findConfigMapDefinitions(document: TextDocument): ConfigMapDefinition[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inData) {
      ...
      if (trimmedLine.length > 0 && lineIndent >= 0 && lineIndent <= currentIndent) {
        inData = false;
      }
    }
  }
  ...
  if (currentKind && currentName && currentNameRange) {
    const keys = dataStartLine >= 0 ? extractDataKeys(lines, dataStartLine, lines.length) : [];
    definitions.push({
      name: currentName,
      kind: currentKind,
      uri: document.uri,
 ...
 */
function extractDataKeys(lines: string[], startLine: number, endLine: number): KeyDefinition[] {
