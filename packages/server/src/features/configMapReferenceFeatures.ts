...
export function findConfigMapReferenceAtPosition(
	...
	if (keyMatch) {
		...
		if (
			position.character >= keyStartCol &&
			position.character <= keyStartCol + keyName.length
		) {
			// Determine the type of reference by looking at surrounding context
			const context = getContextLines(lines, position.line, 5);
...
function detectKeyReference(
	...
	if (contextStr.includes('configMapKeyRef:')) {
		...
		const keyRefMatch = contextStr.match(
			/configMapKeyRef:[\s\S]*?name:\s*([^\s\n]+)/,
		);
		if (keyRefMatch) {
			...
			return {
				...
				keyName,
				kind: 'ConfigMap',
				range: Range.create(Position.create(line, col), Position.create(line, col + keyName.length)),
			};
		}
	}
 ...
 */
export function findAllConfigMapReferences(
	document: TextDocument,
): ConfigMapReference[] {
