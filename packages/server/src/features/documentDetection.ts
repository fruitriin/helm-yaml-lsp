...
const _ARGO_API_GROUP = 'argoproj.io';
...
export function isArgoWorkflowDocument(document: TextDocument): boolean {
  ...
  const hasArgoKind =
    /kind:\s*['"]?(ClusterWorkflowTemplate|WorkflowTemplate|CronWorkflow|Workflow)['"]?/.test(text);
...
function hasChartYaml(filePath: string): boolean {
  // キャッシュチェック
  const cached = chartYamlCache.get(filePath);
  if (cached !== undefined) {
    return cached;
  }
...
export function isHelmTemplate(document: TextDocument): boolean {
  ...
  const isInTemplatesDir = filePath.includes('/templates/') || filePath.includes('\\templates\\');
