 ...
 */
export function findDefineBlocks(
	...
	for (let lineNum = 0; lineNum < lines.length; lineNum++) {
		...
		if (match) {
			...
			for (let i = lineNum; i < lines.length; i++) {
				...
				if (i > lineNum) {
					const nestedDefine = currentLine.match(
						/\{\{-?\s*define\s+"[^"]+"\s*-?\}\}/g,
					);
					if (nestedDefine) {
 ...
 */
function extractDescriptionAboveDefine(
	lines: string[],
	defineLineNum: number,
): string | undefined {
 ...
 */
export function findAllTemplateReferences(
	document: TextDocument,
): HelmTemplateReference[] {
