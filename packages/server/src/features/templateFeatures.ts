 ...
 * }
 */
export function findTemplateDefinitions(
	...
	for (let lineNum = 0; lineNum < lines.length; lineNum++) {
		...
		if (inTemplatesSection) {
			...
			const templateNameMatch = line.match(
				/^(\s*)-\s*name:\s*['"]?([\w-]+)['"]?/,
			);
...
export function findTemplateReferenceAtPosition(
	...
	if (templateRefMatch) {
		...
		for (let i = templateRefLine + 1; i < lines.length; i++) {
			...
			// template: フィールド
			const templateMatch = currentLine.match(
				/^\s*template:\s*['"]?([\w-]+)['"]?/,
			);
			if (templateMatch) {
			...
			// clusterScope: フィールド
			const clusterScopeMatch = currentLine.match(
				/^\s*clusterScope:\s*(true|false)/,
			);
