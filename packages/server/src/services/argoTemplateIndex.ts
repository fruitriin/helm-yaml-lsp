...
export class ArgoTemplateIndex {
	...
	async initialize(): Promise<void> {
		...
		for (const folder of this.workspaceFolders) {
			const uris = await findFiles('**/*.{yaml,yml}', folder, [
				'**/node_modules/**',
				'**/.git/**',
			]);
		...
		console.error(
			`[ArgoTemplateIndex] Initialized with ${this.index.size} WorkflowTemplates`,
		);
	}
		...
		try {
			...
			for (const def of definitions) {
				if (
					...
					def.kind !== 'ClusterWorkflowTemplate'
				) {
					continue;
				}
	...
	removeFile(uri: string): void {
		...
		if (keysToDelete.length > 0) {
			console.error(
				`[ArgoTemplateIndex] Removed ${keysToDelete.length} templates from ${uri}`,
			);
		}
	}
	...
	async findTemplate(
		...
		const kind: ArgoWorkflowKind = clusterScope
