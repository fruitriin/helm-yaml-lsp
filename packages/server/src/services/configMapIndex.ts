...
import type { ConfigMapDefinition, KeyDefinition } from '@/features/configMapFeatures';
...
export class ConfigMapIndex {
  ...
  private async scanWorkspaceFolder(folderPath: string): Promise<void> {
    try {
      ...
      console.error(`[ConfigMapIndex] Error scanning workspace folder ${folderPath}:`, error);
    }
  }
  ...
  removeFile(uri: string): void {
    ...
    console.log(`[ConfigMapIndex] Removed ${definitions.length} definition(s) from ${uri}`);
  }
}
