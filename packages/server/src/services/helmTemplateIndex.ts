...
export class HelmTemplateIndex {
	...
	async indexChartTemplates(chart: HelmChart): Promise<void> {
		...
		try {
			...
			if (!fs.existsSync(chart.templatesDir)) {
				console.log(
					`[HelmTemplateIndex] templates/ directory not found for Chart ${chart.name}`,
				);
			...
			console.error(
				`[HelmTemplateIndex] Error indexing templates for ${chart.name}:`,
				error,
			);
			this.templatesByChart.set(chart.name, templatesMap);
		}
	...
	private findTemplateFiles(directory: string): string[] {
		...
		try {
			...
			console.error(
				`[HelmTemplateIndex] Error reading directory ${directory}:`,
				error,
			);
		}
	...
	async indexTemplateFile(
		...
		try {
			...
			console.error(
				`[HelmTemplateIndex] Error indexing template file ${uri}:`,
				error,
			);
		}
	}
	...
	findTemplate(
		chartName: string,
		templateName: string,
	): HelmTemplateDefinition | undefined {
	...
	async updateTemplateFile(uri: string): Promise<void> {
		...
		if (!chartName) {
			console.log(
				`[HelmTemplateIndex] Cannot update: file not associated with any Chart: ${uri}`,
			);
			return;
		}

		console.log(
			`[HelmTemplateIndex] Updating template file for Chart: ${chartName}`,
		);
