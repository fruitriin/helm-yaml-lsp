...
import { findValueByPath, findValuesByPrefix, parseValuesYaml } from '@/features/valuesYamlParser';
...
export class ValuesIndex {
  ...
  async initialize(charts: HelmChart[]): Promise<void> {
    ...
    for (const chart of charts) {
      if (chart.valuesYamlUri) {
        await this.indexValuesFile(chart.valuesYamlUri, chart.name);
      }
    }

    console.log(`[ValuesIndex] Initialized with ${this.valuesByChart.size} Chart(s)`);
  ...
  async indexValuesFile(valuesUri: string, chartName: string): Promise<void> {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        console.log(`[ValuesIndex] values.yaml not found for Chart ${chartName}: ${filePath}`);
      ...
      console.log(`[ValuesIndex] Indexed ${definitions.length} values from ${chartName}`);
    } catch (error) {
      console.error(`[ValuesIndex] Error indexing values.yaml for ${chartName}:`, error);
      this.valuesByChart.set(chartName, []);
    }
  }
