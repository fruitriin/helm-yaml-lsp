...
`;
		...
		for (let i = 0; i < lines.length; i++) {
			...
			// Check kind match
			const kindMatch = line.match(/^kind:\s*['"]?(ClusterWorkflowTemplate|WorkflowTemplate|CronWorkflow|Workflow)['"]?/);
			if (kindMatch) {
			...
			if (templateNameMatch) {
				console.log(`  → Template name matched: ${templateNameMatch[2]}, indent=${templateNameMatch[1].length}`);
			}
		}
