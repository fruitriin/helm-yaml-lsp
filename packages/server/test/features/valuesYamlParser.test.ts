...
`;
			...
			const imageRepo = definitions.find(
				(d) => d.path === 'image.repository',
			);
...
`;
			...
			const dbHost = definitions.find(
				(d) => d.path === 'app.config.database.host',
			);
...
`;
	...
	describe('findValuesByPrefix', () => {
		...
		it('should find values by prefix', () => {
			...
			expect(results.map((d) => d.path).sort()).toEqual([
				'image',
				'image.repository',
				'image.tag',
			]);
